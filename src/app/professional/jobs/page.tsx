...
import { ImagePlus, PencilLine, Save } from "lucide-react";
  ...
  return (
    <AppShell>
      <div className="mb-6">
        ...
        <h1 className="mt-1 text-3xl font-semibold text-ink">Jobs and applications</h1>
      </div>
      ...
      <section className="mb-8">
        ...
        <div className="space-y-4">
          {applications.map((application) => {
            ...
            return (
              <article className="rounded-lg border border-line bg-white p-5 shadow-sm" key={application.id}>
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <StatusPill tone={application.status === "awarded" ? "green" : "teal"}>{application.status}</StatusPill>
                    <h3 className="mt-2 text-xl font-semibold text-ink">{application.job?.title ?? "Applied job"}</h3>
                ...
                {draft ? (
                  <div className="mt-4 grid gap-4">
                    ...
                    <Button className="w-fit" disabled={savingApplicationId === application.id} onClick={() => save(application)} type="button">
      ...
      <section>
        ...
        <div className="space-y-4">
          {jobs.map((job) => (
            <article className="rounded-lg border border-line bg-white p-5 shadow-sm" key={job.id}>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <StatusPill>{job.categories?.name ?? "General service"}</StatusPill>
                  <h3 className="mt-1 text-xl font-semibold text-ink">{job.title}</h3>
